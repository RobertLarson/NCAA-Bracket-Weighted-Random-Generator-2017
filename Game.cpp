...
void Game::Play(unsigned long roundId) {
  ...
  if (m_pTeamOne != 0)
    team1OddsToAdvance = m_pTeamOne->GetOddsToMakeRound(roundId);
  else
    return;
  double team2OddsToAdvance = 0.0;
  if (m_pTeamTwo != 0)
    team2OddsToAdvance = m_pTeamTwo->GetOddsToMakeRound(roundId);
  else
    return;

  if (team1OddsToAdvance != 0.0 && team2OddsToAdvance != 0.0) {
  ...
  } else if (team1OddsToAdvance != 0.0 && team2OddsToAdvance == 0.0) {
    m_pNextRoundGame->AddTeam(m_pTeamOne);
  } else if (team1OddsToAdvance == 0.0 && team2OddsToAdvance != 0.0) {
    m_pNextRoundGame->AddTeam(m_pTeamTwo);
  } else {
